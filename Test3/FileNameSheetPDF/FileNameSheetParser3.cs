...
namespace AndyShared.FileSupport.FileNameSheetPDF
{
	...
	#region private methods
		...
		{
			if (compStr.Length > SheetPdfManager.Instance.CompLengths[idx]) SheetPdfManager.Instance.CompLengths[idx]=compStr.Length;
		}

		private void initCompLengths(int count)
		{
			SheetPdfManager.Instance.CompLengths = new List<int>(count);

			for (int i = 0; i < count; i++)
			{
				SheetPdfManager.Instance.CompLengths.Add(-1);
			}
		}
