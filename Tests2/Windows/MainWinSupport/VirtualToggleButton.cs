...
{
	public sealed class VirtualToggleButton
    {
        private VirtualToggleButton() { }
        ...
        public static readonly DependencyProperty IsThreeStateProperty = DependencyProperty.RegisterAttached("IsThreeState", typeof(bool), typeof(VirtualToggleButton), new FrameworkPropertyMetadata(Convert.ToBoolean(false)));
        /// <summary>
        ...
        public static readonly DependencyProperty IsVirtualToggleButtonProperty = DependencyProperty.RegisterAttached("IsVirtualToggleButton", typeof(bool), typeof(VirtualToggleButton), new FrameworkPropertyMetadata(Convert.ToBoolean(false), new PropertyChangedCallback(OnIsVirtualToggleButtonChanged)));
        /// <summary>
        ...
        static internal RoutedEventArgs RaiseCheckedEvent(UIElement target)
        {
            if (target == null)
            {
                return null;
        ...
        /// </summary>
        /// <param name="target">UIElement or ContentElement on which to raise the event</param>
        static internal RoutedEventArgs RaiseUncheckedEvent(UIElement target)
        {
            if (target == null)
            {
                return null;
        ...
        /// </summary>
        /// <param name="target">UIElement or ContentElement on which to raise the event</param>
        static internal RoutedEventArgs RaiseIndeterminateEvent(UIElement target)
        {
            if (target == null)
            {
                return null;
        ...
        private static void OnKeyDown(object sender, KeyEventArgs e)
        {
            if (e.OriginalSource == sender)
            {
                ...
                else if (e.Key == Key.Enter && Convert.ToBoolean((sender as DependencyObject).GetValue(KeyboardNavigation.AcceptsReturnProperty)))
                {
                    UpdateIsChecked(sender as DependencyObject);
                    e.Handled = true;
                }
        ...
        private static void UpdateIsChecked(DependencyObject d)
        {
            ...
            if (isChecked == true)
            {
                SetIsChecked(d, GetIsThreeState(d) ? (Nullable<bool>)null : false);
            }
            else
            {
