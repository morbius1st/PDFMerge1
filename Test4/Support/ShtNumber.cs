...
using static Test4.Support.FileNameSheetPdf3.StatusCodes;
using static Test4.Support.FileNameSheetPdf3;
...
namespace Test4.Support
{
	...
	public class ShtNumber : INotifyPropertyChanged
	{
		...
		private FileNameSheetPdf3.StatusCodes statusCode = SC_NONE;
		...
		public FileNameSheetPdf3.StatusCodes StatusCode
		{
			get => statusCode;
			private set
			{
		...
		public bool ParseShtNumber()
		{
			...
			Match match = FileNameSheetParser3.Instance.MatchShtNumber(origSheetNumber);

			if (!match.Success)
			{
				StatusCode = FileNameSheetPdf3.StatusCodes.SC_PARSER_MATCH_FAILED;
				return false;
			}
			...
			if (!FileNameSheetParser3.Instance.extractShtNumComps3(this, g))
			{
				StatusCode = FileNameSheetPdf3.StatusCodes.SC_PARSER_COMP_EXTRACT_FAILED;
				return false;
			}
		...
		private bool okToParse()
		{
			...
			{
				StatusCode = FileNameSheetPdf3.StatusCodes.SC_VOID_SHTNUM;
				return false;
			}

			if (!FileNameSheetParser3.Instance.IsConfigd)
			{
				StatusCode = FileNameSheetPdf3.StatusCodes.SC_PARSER_NOT_CONFIGD;
				return false;
			}
